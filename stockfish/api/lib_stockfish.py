...
class Stockfish:
    ...
    def receive_text(self, exit_text = "Checkers:", max_lines = 100):
        """ Generic method for caputring (mutli-) line output 
            returns lines stripped of carriage return           """
        lines = []
        for i in range(max_lines):
            try:
                ...
                return lines
            lines.append(line)
            if line.find(exit_text) > -1:
                return lines
        return lines
    
    def get_position(self):
        self.__put('d')
        txt = self.receive_text()
        return txt
